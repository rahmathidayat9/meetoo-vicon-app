...
// Asynchronous function to fetch user information from a server
async function validatingMeet() {
    try {

        if (window.localStorage.user_id === undefined) {
            Swal.fire({
                title: 'Error!',
                text: 'Autorisasi meeting tidak valid',
                icon: 'error',
                confirmButtonText: 'OK'
            })
        ...
        const lastSegment = urlSegments.filter(segment => segment.trim() !== '').pop();

        console.log('End of URL segment:', lastSegment);


        // Make an AJAX request using fetch to get user data
        const response = await fetch('/validating-meet', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json', // Correct header to indicate JSON data
            },
            body: JSON.stringify({
                meeting_url: lastSegment,
                user_id: user_id,
                user_name: user_name,
                key: "aaaaa", // The actual data being sent
            }), // Body should be outside of headers
        });
        ...
        // return userInfo; // Return the fetched user info
    } catch (error) {
...
async function whoAreYou() {
    ...
    const userInfo = await validatingMeet();
    ...
    Swal.fire({
        ...
        inputValue: window.localStorage.user_name ? window.localStorage.user_name : '',
...
function leaveRoom() {
    ...
    Swal.fire({
        title: "Thanks from Meetoo!",
        text: "Redirect dalam 2 detik",
        icon: "success"
    });
...
function redirectOnLeave() {
    redirectActive ? openURL(redirectURL) : openURL('/auth-logout');
}
