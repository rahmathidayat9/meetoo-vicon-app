...
// Configure express-session with a 24-hour max age
...
app.set('layout', path.join(__dirname, '..', '..', 'public', 'views', 'template'), {
    test: "OKOKOK",
})
app.set('view engine', 'ejs')
app.use(expressLayouts);

// Set the views directory to the correct path relative to 'app.js'
...
app.post('/auth-login', async (req, res) => {
    ...
    try {
        ...
        const [rows, fields] = await database.execute(queryString);
...
app.get('/dashboard', async (req, res) => {
    ...
    const query_meetings_count = `SELECT COUNT(*) AS total_meeting FROM meetings`
    const [rowsMeeting, fieldsMeeting] = await database.query(query_meetings_count);
    const query_users_count = `SELECT COUNT(*) AS total_user FROM users`
    const [rowsUser, fieldsUser] = await database.query(query_users_count);

    let total_meeting = 0;
    let total_user = 0;

    if (rowsMeeting && rowsUser != null) {
        total_meeting = rowsMeeting[0].total_meeting
        total_user = rowsUser[0].total_user
    }
...
app.get('/users-management', async (req, res) => {
    ...
    res.render('dashboard/users-management', {
...
app.post('/users-management/add', async (req, res) => {
    ...
    try {
        const sql = 'INSERT INTO `users`(`name`, `username`, `password`, `role`) VALUES ("'+name+'", "'+username+'", "'+password+'", "'+role+'")';
...
app.get('/meetings-management-admin', async (req, res) => {
    const name = req.session.name
    const userId = req.session.userId
    const role = req.session.role
    
    /* Meetings Lists */
    let  queryString = `SELECT * FROM meetings`
    const [rows, fields] = await database.execute(queryString);

    res.render('dashboard/meetings-management-admin', {
        title: 'Meetings',
        data: {
            meetings: rows,
        },
...
app.get('/meetings-management-user', async (req, res) => {
    const name = req.session.name
    const userId = req.session.userId
    const role = req.session.role
    ...
    res.render('dashboard/meetings-management-user', {
        title: 'Meetings',
        data: {
            meetings: rows,
        },
...
function generateRandomToken() {
    const characters = 'abcdefghijklmnopqrstuvwxyz0123456789'; // All possible characters
    const tokenLength = 9; // Length of the desired token
    let token = '';

    // Generate a 9-character token by picking random characters
    for (let i = 0; i < tokenLength; i++) {
        const randomIndex = Math.floor(Math.random() * characters.length); // Random index
        token += characters[randomIndex]; // Add the random character to the token
    }

    return token; // Return the generated token
}

app.post('/meetings-management/add', async (req, res) => {
    let meeting_title = req.body.title
    let meeting_token = generateRandomToken();
    let meeting_url = createSlug(meeting_title)

    try {
        const sql = 'INSERT INTO `meetings`(`meeting_title`, `meeting_token`, `meeting_url`) VALUES ("'+meeting_title+'", "'+meeting_token+'", "'+meeting_url+'")';
        const [result, fields] = await database.query(sql);
...
app.get('/users-management/delete/:id', async (req, res) => {
    const id = req.params.id
    let  queryString = `DELETE FROM users WHERE id=${id}`
    await database.execute(queryString);

    res.redirect('/users-management')
});

app.get('/meetings-management/delete/:id', async (req, res) => {
    const id = req.params.id
    let  queryString = `DELETE FROM meetings WHERE id=${id}`
    await database.execute(queryString);

    res.redirect('/meetings-management-admin')
});

app.post('/validating-meet', async (req, res) => {
    let meeting_url = req.body.meeting_url
    ...
    let queryStringUser = `SELECT * FROM users WHERE id=${user_id}`
    const [rowsUser, fieldUser] = await database.execute(queryStringUser);

    let queryStringMeeting = `SELECT * FROM meetings WHERE meeting_url='${meeting_url}'`
    const [rowsMeeting, fieldMeeting] = await database.execute(queryStringMeeting);

    if (rowsUser.length > 0) {
        if (req.session.userId != null) {
            if (rowsMeeting.length > 0) {
                res.send({
                    message: 'Meet validating success',
                    status: 'success' 
                })    
            } else {
                req.session.destroy();
